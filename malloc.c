...
#define MMAP(addr, size, prot, flags) ((dev_zero_fd < 0) ? \
 (dev_zero_fd = open("/dev/zero", O_RDWR), \
  smmap((addr), (size_t)(size), (prot), (flags), dev_zero_fd, 0)) : \
   smmap((addr), (size_t)(size), (prot), (flags), dev_zero_fd, 0))

#else

#define MMAP(addr, size, prot, flags) \
 (smmap((addr), (size_t)(size), (prot), (flags)|MAP_ANONYMOUS, -1, 0))
...
#endif
{
  ...
  if (mem != 0) {
    ...
    */
      ...
      smunmap((char*)p - offset, size + offset);   //!!!!!!!!!!Mark!!!!!!!!!!
#endif
    }
  }
}
